...
> 2026-07-20 update: OKX is now the primary exchange route. Hyperliquid and Binance remain legacy optional adapters. New market-data, demo-execution, and live-readiness work should target OKX first.
...
后续升级时优先从 `upstream/main` 吸收原项目的 agent、LLM、CLI、结构化输出改进；我们的 Hyperliquid 交易中心扩展尽量放在 `tradingagents/crypto/` 和 `docs/`，减少和上游股票分析代码冲突。

## 底层框架约束

- 上游 TradingAgents 的 agent 角色、LangGraph 结构、LLM factory、结构化输出 helpers、CLI 风格、报告/记忆约定是底层能力。
- Hyperliquid、Hermes、Lana-inspired 策略、hotlist、执行器都是扩展层，默认放在 `tradingagents/crypto/`。
...
- 股票行情数据替换为 Hyperliquid K线、中间价、市场规则和个人账户状态。
- Fundamentals Analyst 暂不用于币种基本面，第一阶段以技术、成交量、波动率和风险为主。
- Sentiment Analyst 默认把情绪视为未知，除非后续明确接入新闻、社媒或链上数据。
- Trader 初始阶段只能提出 long-only `BUY/HOLD/REJECT`，不允许做空，Hyperliquid 杠杆上限固定为 `1`。
- Portfolio Manager 的最终意见仍必须经过确定性风控，不能直接下单。

## 当前代码落点

- `tradingagents/crypto/hyperliquid_client.py`：Hyperliquid public/account REST、市场元数据、K线和账户状态。
- `tradingagents/crypto/binance_client.py`：旧 Binance Spot 适配器参考，不再是主交易所路线。
...
2. 增加自动热度采集源，把 X、Hyperliquid 生态讨论、论坛、新闻、链上热度文本喂给 `crypto-attention-ingest`。
3. 增加用户数据流和订单状态恢复。
4. 增加持仓状态机、止损止盈和急停熔断。
5. 增加回测和纸面交易绩效统计。
